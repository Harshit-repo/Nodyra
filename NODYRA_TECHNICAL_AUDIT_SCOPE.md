...
Assess Nodyra for four release gates and produce actionable, evidence-based
...
Nodyra is a **Python-first, self-hostable workflow automation platform** (n8n-in-spirit,
...
| Execution engine core | `packages/core/nodyra` | 24 files, ~6.6k LOC |
| Node library | `packages/nodes/nodyra_nodes` | 86 files, ~34.3k LOC |
...
workflow execution engine (`packages/core/nodyra/engine`); node system; durable
...
Node drag; edge creation (`NodyraEdge`); node config (Inspector/NDV);
...
### 5. Node System
Built-in nodes; node schema/contract (`packages/core/nodyra/sdk.py`, `models.py`);
...
import restrictions; expression sandbox (`nodyra/expr.py`, `expr_preview_worker.py`).
...
- `NODYRA_TECHNICAL_AUDIT_SCOPE.md` (this file)
- `NODYRA_REPOSITORY_MAP.md`
...
- `NODYRA_AUDIT_FINAL_REPORT.md` (master report)
