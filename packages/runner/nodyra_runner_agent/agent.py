"""Nodyra runner agent — CLI + run loop.

    nodyra-runner register --api-url URL --token TOKEN [--name NAME]
...
from nodyra_runner_agent.ws_client import AgentWSClient
...
def _cmd_register(args: argparse.Namespace) -> None:
    ...
    print("Run 'nodyra-runner start' to begin accepting workflow runs.")
...
def _cmd_start(args: argparse.Namespace) -> None:
    ...
    if cfg is None:
        raise SystemExit(
            "No config found. Run 'nodyra-runner register ...' first."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="nodyra-runner")
    sub = parser.add_subparsers(dest="command", required=True)

    reg = sub.add_parser("register", help="register this machine as a runner")
    reg.add_argument("--api-url", required=True, help="Nodyra API base URL")
