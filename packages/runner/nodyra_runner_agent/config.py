"""Agent config persistence under ``~/.nodyra-runner/``.

``nodyra-runner register`` writes ``config.json`` with the API URL, the
runner id, and the registration token; ``nodyra-runner start`` reads it.
...
def data_dir() -> Path:
    raw = os.environ.get("NODYRA_RUNNER_DATA", "~/.nodyra-runner")
...
class AgentConfig:
    ...
    def wheel_index_url(self) -> str:
        """``--find-links`` page serving the unpublished nodyra-* wheels."""
