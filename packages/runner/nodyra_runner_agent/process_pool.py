"""Run a single workflow in a freshly-spawned ``nodyra_runtime`` subprocess.
...
async def run_workflow_subprocess(
    ...
    """Spawn ``nodyra_runtime``, run the workflow, return the status string."""
    ...
    if proc.stdin is None or proc.stdout is None:
        raise RuntimeError("nodyra_runtime subprocess pipes were not opened")
    ...
    try:
        ...
        if not ready_line:
            raise RuntimeError("nodyra_runtime did not emit a ready event")
        ...
        while True:
            ...
            if not line:
                raise RuntimeError("nodyra_runtime closed stdout before result")
            try:
