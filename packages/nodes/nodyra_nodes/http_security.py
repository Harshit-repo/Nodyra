...
_ALLOW_PRIVATE_ENV = "NODYRA_ALLOW_PRIVATE_EGRESS"
_LEGACY_ALLOW_PRIVATE_ENV = "NOODLE_ALLOW_PRIVATE_EGRESS"
...
def private_egress_allowed() -> bool:
    ...
    a VPC database) set ``NODYRA_ALLOW_PRIVATE_EGRESS=1`` to opt out.

    Scheme validation (http/https only) always applies regardless of this flag.
    """
    return (
        _env_with_legacy(_ALLOW_PRIVATE_ENV, _LEGACY_ALLOW_PRIVATE_ENV).strip().lower() in _TRUTHY
    )
...
def assert_public_host(host: str, port: int | None = None, *, context: str = "connection") -> None:
    ...
    connection nodes). Respects the ``NODYRA_ALLOW_PRIVATE_EGRESS`` opt-out so a
    ...
    for info in infos:
        ...
        if sockaddr and _is_private_address(str(sockaddr[0])):
            raise UnsafeHttpTargetError(
                f"{context}: host resolves to a private, loopback, or link-local address"
            )
...
def assert_public_http_url(url: str, *, context: str = "HTTP request") -> None:
    ...
    if parsed.scheme.lower() not in {"http", "https"}:
        raise UnsafeHttpTargetError(f"{context}: only http and https URLs are allowed")
...
def _strip_sensitive_headers(body_kwargs: dict[str, Any]) -> None:
    ...
    if not headers:
        return
    body_kwargs["headers"] = {
        key: value for key, value in dict(headers).items() if key.lower() not in _SENSITIVE_HEADERS
    }
