...
def activate_websocket(
    ...
    """Register the WebSocket path with the Noodle runtime."""
    params = ctx.params
...
def handle_websocket_event(
    ...
    client_id = request.headers.get("x-noodle-client-id", "")
    event_type = request.headers.get("x-noodle-ws-event", "message")
...
WEBSOCKET_TRIGGER_SPEC = ProviderTriggerSpec(
    ...
    description=(
        ...
        "Noodle opens a WebSocket endpoint at /ws/triggers/{path}."
    ),
    icon="globe",
    params=(
        OperationParamSpec(
