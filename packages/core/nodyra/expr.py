...
# Mirror of _CODE_NODE_BLOCKED_IMPORTS in nodyra_nodes/builtin.py.
CODE_NODE_BLOCKED_MODULES: frozenset[str] = frozenset({
    ...
    # Network escape paths (use Nodyra's HTTP / integration nodes instead)
    "socket", "ssl", "urllib", "http", "ftplib", "telnetlib",
    "smtplib", "imaplib", "poplib", "xmlrpc",
    # File system escape paths (use Nodyra's file nodes instead)
...
def _is_dataset_envelope(value: Any) -> bool:
    """Detect a DatasetRef envelope without importing ``nodyra.datasets``.

    Kept local to avoid an import cycle (``datasets`` imports from core
    modules). Mirrors the marker contract in :mod:`nodyra.datasets`.
    """
    return (
        isinstance(value, dict)
        and value.get("__nodyra_dataset__") is True
