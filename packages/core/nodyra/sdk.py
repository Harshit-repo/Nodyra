...
from nodyra.models import CredentialSpec, NodeManifest, ParamSpec, PortSpec, SystemRequirement
...
# id; the function's ``__nodyra_node__`` metadata is read back afterwards and
# registered under a namespaced id instead.
_suppress_registration: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "nodyra_suppress_node_registration", default=False
)
...
def register_module_functions(
    ...
    *explicit mode*: only decorated functions are registered as nodes (their
    ...
    # attaches ``__nodyra_node__`` metadata; we register it ourselves below
    ...
    for key in new_keys:
        ...
        decorator_def: NodeDef | None = getattr(value, "__nodyra_node__", None)
...
def node(
    *,
    ...
    """Register a function as a Nodyra node.
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        func.__nodyra_node__ = node_def  # type: ignore[attr-defined]
