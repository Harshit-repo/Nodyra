...
:class:`nodyra.artifacts.LocalArtifactStore` artifact (typically a Parquet
...
implementations live in ``nodyra_nodes.datasets`` to keep the heavy
...
# ``nodyra_nodes`` package to keep the heavy dependency out of core. The nodes
...
def register_materializer(fn: _Materializer) -> None:
    """Register the DuckDB-backed row materializer (called by nodyra_nodes)."""
...
def register_dataset_writer(fn: _DatasetWriter) -> None:
    """Register the DuckDB-backed records->DatasetRef writer (called by nodyra_nodes)."""
...
def dataset_from_records(
    ...
    if _dataset_writer is None:
        raise RuntimeError(
            "no dataset writer registered; import nodyra_nodes.datasets"
        )
...
def materialize_dataset_rows(
    ...
    *,
    ...
    if _materializer is None:
        raise RuntimeError(
            "no dataset materializer registered; import nodyra_nodes.datasets"
        )
