...
Unlike :func:`nodyra_exporter.codegen.workflow_to_script` (which embeds the
...
Nodyra engine — execution semantics are identical to running inside Nodyra.
"""
...
_HEADER_TEMPLATE = '''"""Nodyra workflow: __NAME__ (code-first export)
...
from nodyra.sdk import registry as builtin_registry
...
def workflow_to_module(
    ...
    *,
    ...
    ``registry`` is the live node registry (``nodyra.sdk.registry`` with
    ``nodyra_nodes`` imported) used to look up manifests, input ports, and
    ...
    for graph_node in graph.nodes:
        ...
        block = (
            ...
            f"    \"\"\"{graph_node.type} node from the Nodyra workflow.\"\"\"\n"
