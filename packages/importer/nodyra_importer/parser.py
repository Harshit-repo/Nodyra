"""AST-based importer: Nodyra .module.py → WorkflowGraph.

Parses the structured output of nodyra_exporter.workflow_to_module() without
...
def import_module(source: str) -> WorkflowGraph:
    """Parse a Nodyra .module.py source string into a WorkflowGraph.

    Raises ImportError with a descriptive message if the source is not a
    valid Nodyra module export. Never executes the source code.
    """
    try:
    ...
    if not nodes:
        raise ImportError(
            ...
            "The file does not look like a Nodyra .module.py export."
        )
