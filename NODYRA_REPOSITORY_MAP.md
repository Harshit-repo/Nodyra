...
packages/
  core/     nodyra/ — execution engine, models, expr sandbox, SDK, serialization
  nodes/    nodyra_nodes/ — built-in node library (86 files) + integrations_v2
  runner/   nodyra_runner_agent/ — remote runner agent (WebSocket)
  runtime/  nodyra_runtime/ — shared runtime helpers
  exporter/ nodyra_exporter/ — code-first workflow export
...
### Execution engine (`packages/core/nodyra`)
...
- **Editor/canvas (`src/editor`):** `Canvas` (xyflow), `NodeCard`, `NodeDetails`/
  `NodeDetailModal`, `Inspector`, `NodePalette`, `NodyraEdge`, `LoopFrame`,
