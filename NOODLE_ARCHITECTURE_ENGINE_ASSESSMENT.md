# Noodle Architecture & Engine Assessment
...
│   ├── models.py               # WorkflowGraph, GraphNode, Edge, RunResult, NoodleItem
...
A `NoodleItem` wrapper is defined (n8n-style `{json, meta, binary_data}`) but is NOT universally applied — nodes return plain Python values. The engine wraps/unwraps transparently at the `_normalize_outputs` boundary.
...
Noodle does NOT use Celery. The queue system is:
...
5. **NoodleItem not universally adopted**:
   - The `NoodleItem` wrapper (n8n-style `{json, meta, binary_data}`) is defined but not applied. Nodes return plain values.
   - **Impact**: No provenance tracking (which node produced this item? at what time?). Binary data handling is ad-hoc.
   - **Fix**: Either fully adopt NoodleItem wrapping or remove it to reduce confusion.
...
- **Severity**: LOW
- **File**: `packages/core/noodle/engine/scheduler.py:377` (`execute`)
...
- **Severity**: LOW
- **File**: `packages/core/noodle/engine/scheduler.py:166` (`_build_plan`)
...
- **Severity**: LOW
- **File**: `packages/core/noodle/engine/scheduler.py:56` (`_predecessors`)
...
- **Severity**: MEDIUM
- **File**: `packages/core/noodle/sdk.py` → `registry = NodeRegistry()` (global)
- **Current behavior**: Tests that register custom nodes leak into subsequent tests.
- **Status**: Open (mitigation: tests use isolated registries via `make_registry()`, but the global `registry` is still used by metanode tests via `import noodle_nodes`)

### B-6: `_install_capture` is Process-Wide
- **Severity**: DOC
- **File**: `packages/core/noodle/engine/node_exec.py:192-199`
...
- **File**: `packages/core/noodle/engine/validation.py:270-280`
...
- **Severity**: IMPROVEMENT
- **File**: `packages/core/noodle/engine/validation.py:266-269`
...
- **Severity**: P1
- **File**: `packages/core/noodle/engine/scheduler.py:47`
...
**Fit for Noodle:** Excellent. The current architecture is 80% correct.
...
**Fit for Noodle:** Medium. The internal modules are already well-factored. The problem is in `runner.py`, not the engine.
...
**Fit for Noodle:** Poor. Noodle's value prop is Python-native, self-hostable simplicity. Adding Temporal as a hard dependency contradicts this. However, Temporal-style durable execution semantics can be adopted without the Temporal infrastructure.
...
**Cons:**
- Noodle already HAS a DB-backed durable queue that works
- Celery specifically adds significant operational complexity
- The current queue (RunQueueEntry) is simpler and more correct for Noodle's needs
- Adding a second queue would create inconsistency

**Fit for Noodle:** Poor for Celery. Unnecessary for others — the DB-backed queue already does what RQ/Arq would provide.
...
**Fit for Noodle:** Excellent. This is already implemented via `use_subprocess_runner` and `dispatch_role` settings.
...
### Core Components (all in `packages/core/noodle/engine/`)
...
Prefect's execution semantics provide several patterns Noodle should adopt
...
Noodle currently stores node outputs as JSON blobs in `node_runs.output` —
...
Noodle's `on_error=continue` boolean is too coarse. A hook system would let
nodes:
...
Prefect limits concurrency per task *type*. Noodle's `max_node_concurrency` is
...
Noodle has port kind validation (`ai_language_model`, `dataset`, etc.) but
...
Prefect uses `idempotency_key` to prevent duplicate flow runs. Noodle has
...
- **The scheduling server** — Noodle's DB-backed `RunQueueEntry` is simpler
  and more correct for this scale
- **The flow-of-flows orchestration** — Noodle's subworkflow engine
  (`engine/subworkflows.py`) handles nesting cleanly
- **The UI/Cloud platform** — Noodle has its own frontend
- **Work pools / agents** — Noodle's `RunnerPool` + `RunExecutor` protocol
  already covers this
...
| 2 | Adopt NoodleItem wrapping universally (or remove it) | Medium |
...
## Decision: Is Current Architecture the Best for Noodle?

**Answer: Yes, keep and improve.**

The current architecture is fundamentally correct. The DAG engine, DB-backed queue, executor abstraction, and multi-tenancy design are all well-chosen for Noodle's product direction.
...
3. **Do NOT add Temporal/Prefect** — they add operational complexity that contradicts Noodle's self-hosted simplicity
...
1. **Microservices** — Noodle's monolith-with-worker architecture is correct for its scale. Don't split into microservices until you have >1000 concurrent workflows.
...
Noodle is a **solid beta-quality workflow platform**. The architecture is well-designed, the engine is correct, and the multi-tenancy is sophisticated. The remaining gaps are execution durability, validation completeness, and observability — all addressable with incremental work.
...
2. Adopt NoodleItem wrapping or remove it
...
**Noodle is now beta-ready for self-hosted single-tenant deployment. Multi-tenant hosted requires sandbox hardening.**
...
| P2 | Adopt NoodleItem wrapping universally or remove it |
...
### Engine Core
- `packages/core/noodle/engine/__init__.py` — Public API exports
- `packages/core/noodle/engine/scheduler.py` — Topological sort, planning, execution orchestration
- `packages/core/noodle/engine/node_exec.py` — Single-node execution, retries, timeouts, isolation
- `packages/core/noodle/engine/loops.py` — Loop region discovery, for-each/while/until drivers
- `packages/core/noodle/engine/metanodes.py` — Transparent metanode expansion
- `packages/core/noodle/engine/subworkflows.py` — Sub-workflow calls
- `packages/core/noodle/engine/validation.py` — Port kind validation
- `packages/core/noodle/engine/agent.py` — AI agent action dispatch
- `packages/core/noodle/engine/datasets.py` — Dataset auto-expand/promote
- `packages/core/noodle/engine/types.py` — EventCallback, GraphError
...
### SDK & Models
- `packages/core/noodle/sdk.py` — @node decorator, NodeRegistry
- `packages/core/noodle/models.py` — WorkflowGraph, GraphNode, Edge, RunResult, etc.
- `packages/core/noodle/context.py` — ContextVars for execution context
- `packages/core/noodle/serialization.py` — Value serialization

### Runners
- `packages/runner/noodle_runner_agent/agent.py` — Runner agent (remote execution)
- `packages/runner/noodle_runner_agent/ws_client.py` — WebSocket client for runner→API
- `packages/runtime/noodle_runtime/server.py` — HTTP runtime server
