...
async def _require_org_role(
    session: AsyncSession, user: User, org_id: str, minimum: str
) -> str:
...
async def create_org(
    ...
    await log_audit(
        session, "create", "organization", org.id, f"{org.name} ({slug})",
    ...
    try:
        ...
        raise HTTPException(status.HTTP_409_CONFLICT, "Slug is already taken.")
    return OrgInfo(
        id=org.id, name=org.name, slug=org.slug, status=org.status, role="owner"
    )
...
async def update_org(
    ...
    await session.commit()
    return OrgInfo(
        id=org.id, name=org.name, slug=org.slug, status=org.status, role=role
    )
...
async def delete_org(
    ...
    if org is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Organization not found")
    await log_audit(
        session, "delete", "organization", org.id, org.name,
        actor_id=user.id, actor_email=user.email,
    )
...
async def add_member(
    ...
    if role == "owner" and actor_role != "owner":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "Only an owner can add another owner."
        )
    ...
    await log_audit(
        session, "member_add", "organization", org_id, f"{user.email} ({role})",
        actor_id=actor.id, actor_email=actor.email,
    )
    await session.commit()
    return OrgMemberInfo(
        user_id=user.id, email=user.email, name=user.name, role=role
    )
...
async def update_member(
    ...
    if (member.role == "owner" or role == "owner") and actor_role != "owner":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "Only an owner can manage owner roles."
        )
    if (
        member.role == "owner"
        and role != "owner"
        and await _org_owner_count(session, org_id) <= 1
    ):
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST,
    ...
    await log_audit(
        session, "member_role", "organization", org_id,
...
@router.delete(
    "/orgs/current/members/{user_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def remove_member(
    ...
    if member.role == "owner":
        if actor_role != "owner":
            raise HTTPException(
                status.HTTP_403_FORBIDDEN, "Only an owner can remove an owner."
            )
    ...
    user = await session.get(User, user_id)
    await log_audit(
        session, "member_remove", "organization", org_id,
