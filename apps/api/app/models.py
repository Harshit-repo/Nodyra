...
class Workflow(Base):
    ...
    checks: Mapped[list["WorkflowCheck"]] = relationship(
        back_populates="workflow",
        cascade="all, delete-orphan",
        order_by="WorkflowCheck.created_at",
    )


class WorkflowCheck(Base):
    """A persisted, re-runnable workflow check generated by AI or authored by a user."""

    __tablename__ = "workflow_checks"
    __table_args__ = (
        Index("ix_workflow_checks_org_workflow_created", "org_id", "workflow_id", "created_at"),
    )
    ...
    name: Mapped[str] = mapped_column(String(200), nullable=False, default="Workflow check")
    input_data: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    expected_outputs: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    ...
    workflow: Mapped[Workflow] = relationship(back_populates="checks")
