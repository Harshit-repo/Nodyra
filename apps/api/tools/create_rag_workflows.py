...
def make_edge(eid, src, src_out, tgt, tgt_in):
    return {"id": eid, "source": src, "source_output": src_out, "target": tgt, "target_input": tgt_in}
...
wf6_graph = {
    "nodes": [
        ...
        make_node("docs", "code", label="Sample Documents",
                  params={"code": DOCS_CODE}, x=240, outputs_override=["main"]),
        make_node("ds", "records_to_dataset", x=480),
        make_node("chunk", "document_chunk", x=720,
                  params={
                      "text_column": "text",
                      "strategy": "sentence",
                      "chunk_size": 200,
        ...
        make_node("qa", "code", label="Simulated Q&A Pairs",
                  params={"code": QA_PAIRS_CODE}, x=1000, outputs_override=["main"]),
        make_node("qa_ds", "records_to_dataset", x=1240),
        make_node("relevance", "rag_context_relevance", x=1480,
                  params={
        ...
        make_node("answer_eval", "rag_answer_eval", x=1760,
                  params={
                      "mode": "contains_answer",
                      "answer_column": "answer",
                      "expected_column": "expected",
                  }),
        make_node("gate", "eval_gate", x=2040,
                  params={"metric": "accuracy", "operator": ">=",
                          "threshold": 0.6, "on_fail": "branch"}),
        make_node("report", "eval_report", x=2300, y=60,
                  params={"title": "RAG Answer Eval Report", "include_sample_rows": True}),
        make_node("fail", "code", label="Low RAG Accuracy",
                  params={"code": "acc = input.get('input', {}).get('value', 0)\nprint(f'RAG accuracy too low: {acc:.1%}')\noutput = input"},
                  x=2300, y=200, outputs_override=["main"]),
    ],
    "edges": [
...
wf7_graph = {
    "nodes": [
        make_node("trig", "manual_trigger", params={"data": {}}, x=0),
        make_node("gen", "code", label="Generate Multi-Doc Corpus",
                  params={"code": MULTI_DOCS_CODE}, x=240, outputs_override=["main"]),
        make_node("ds", "records_to_dataset", x=480),
        make_node("experiment", "rag_chunking_experiment", x=720,
                  params={
                      "text_column": "text",
        ...
        make_node("done", "code", label="Log Results",
                  params={"code": "n = input.get('n_experiments', 0)\nprint(f'Chunking experiment complete: {n} configurations tested')\noutput = input"},
                  x=1000, outputs_override=["main"]),
    ],
    "edges": [
