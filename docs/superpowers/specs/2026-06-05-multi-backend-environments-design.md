...
| 2 | conda/micromamba backend + system_requirements on `@node` | Wave 3 ML nodes — system deps become real blockers |
| 3 | Docker backend (all three sub-modes) + context-aware system req UI | First enterprise deployment request |

pixi is deferred indefinitely — it is redundant with conda for Noodle's use case and can be added as a community contribution following the same backend protocol.
...
```python
backend: str = "venv"    # "venv" | "conda" | "docker"
backend_config: dict = {}
```
...
`packages: list[str]` is used by venv, conda, and Docker managed mode. Ignored for Docker dockerfile and pre-built.
...
|-------------------|---------|--------|
| venv / conda | "Must be installed on the server" | Shows apt/brew/Windows commands |
...
For venv/conda envs, a footer nudge reads: "Switch to a Docker (Dockerfile) environment to manage this inside Noodle."
...
Creation dialog: tabbed backend selector at the top. Tabs: **uv + venv** | **conda** | **Docker** (greyed out if unavailable per `/environments/backends`). Fields below the tabs update per backend:
...
Environment cards show a backend badge: green `venv`, blue `conda`, purple `docker`.

### `PackageDrawer.tsx`

Channels field rendered above packages for conda environments. Each channel shows which channel resolved each installed package (from `micromamba list --json` output).
