...
### Alembic migrations review (37 migrations + env.py)
- **Chain integrity — clean.** Single root (`0001_baseline`) → single head
  (`0038_timestamp_not_null`), strictly linear, no forks, no duplicate revision
  ids, all reachable. Verified `alembic upgrade head → downgrade base → upgrade
  head` applies end-to-end on SQLite.
- **SQLite-incompatible ALTERs use `op.batch_alter_table`** in upgrades; NOT NULL
  column adds all carry a `server_default` (0023 `allow_concurrent`, 0028
  `include_undecorated`); the only data migration is an **idempotent** singleton
  `system_settings` insert guarded by a table-existence check; several migrations
  use `_tables()/_columns()` guards for re-run safety. Good hygiene overall.
- **env.py hardening:** added `render_as_batch=True` to both `context.configure`
  calls. No effect on Postgres (prod); makes local-SQLite dev and downgrades
  work (bare `op.drop_column` in some `downgrade()` bodies would otherwise fail
  on SQLite) and keeps future autogenerated migrations SQLite-safe.

### ALM-1 — Migration timestamp columns nullable vs NOT-NULL models (Low)
- **Evidence:** `0020` (run_queue) and `0034` (provider_trigger_subscriptions)
  define `created_at`/`updated_at` with a `server_default` but omit
  `nullable=False`, so the DB allows NULL while the ORM models declare them NOT
  NULL (`Mapped[datetime]`). Surfaced by `alembic check` (`modify_nullable`).
  Benign in practice — the server_default means they're never NULL — but it's a
  real model↔schema divergence.
- **Fix:** migration `0038_timestamp_not_null` sets the 4 columns NOT NULL (safe:
