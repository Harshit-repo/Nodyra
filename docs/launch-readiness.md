...
Then the deployment runbook was worked against a live stack, and inside one
afternoon that turned up four production bugs: every freshly built environment
failed to start, and two of sixteen starter templates were broken — one of them
silently, producing nothing while erroring nowhere.

Nothing was wrong with the tests. They run against a *locked* dependency set and
a SQLite database. The product runs against *freshly resolved* dependencies and
PostgreSQL. Every one of those four bugs lived in the gap between those two
worlds.

That gap is now much narrower. But a codebase earns a 10 by surviving contact
with reality, and Nodyra has not had any yet: no production traffic, no external
pentest, no live payment, and not one green CI run in a fortnight.
...
The tag points at `2dbbb213`. The two fixes that matter most landed *after* it:

- `fd3a56b0` — every freshly built workflow environment failed to start
- `1cb92451` — two starter templates were broken

As it stands, v1.0.0 is a release in which a new user's first workflow cannot
run. Nothing was ever published from it (the release job never started), so
moving the tag costs nothing and misleads nobody.
...
| Docs & website | 9 | A public repository, so the links and the fair-code claim stop being false. |
...
| Testing & CI | 7 | A green CI run. The suite also passed while four production bugs were live — that is the real cap, not the billing outage. |
