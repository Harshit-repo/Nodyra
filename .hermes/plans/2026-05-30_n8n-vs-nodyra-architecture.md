# n8n vs Nodyra — Architecture & Backend Comparison
**Date:** 2026-05-30
**Purpose:** Find what Nodyra can adopt from n8n; identify gaps to close.

---

## 1. EXECUTION ENGINE

| Dimension | n8n | Nodyra | Gap |
|-----------|-----|--------|-----|
...
| Retry logic | Manual re-run from UI; no auto-retry in engine | Per-node retry with exponential backoff + jitter | Nodyra ahead |
...
- **Process-pool for code nodes**: the task runner pattern. A pool of pre-warmed Python subprocesses each running `nodyra_runtime`. Broker assigns tasks via an internal queue. V8's `runInContext({timeout})` equivalent in Python is `multiprocessing` with `Process.kill()` after a timeout join.
...
| Job payload | Full workflow definition serialised into queue job | `run_id` only — runner fetches graph from DB | Nodyra better (smaller payload, less Redis) |
...
| Upstream node reference | `$node["NodeName"].json[0].field` | `$node["node_id"].port_name` | Nodyra uses IDs not names |
...
### What to import from n8n
- **Typed item envelope**: define a `NodyraItem` dataclass:
  ```python
  @dataclass
  class NodyraItem:
  ...
  ```
  Node inputs/outputs become `list[NodyraItem]` instead of raw values. This enables per-item error handling, binary passthrough, and item lineage — the foundation for n8n's most powerful features.
...
| Event replay | Not built in (UI reconnects and re-fetches from DB) | YES — `_events` buffer replays missed events | Nodyra ahead |
...
### What to import from n8n
This is a large feature area. For Nodyra's current scope (single-owner + QA accounts), the minimum viable RBAC is:
...
- **Lazy loading**: in `packages/nodes/__init__.py`, instead of `from .builtin import *`, use a `_NODE_MODULES` dict mapping `node_type_id → "nodyra_nodes.builtin"`. Import the module only when a workflow containing that node type is first executed. Cuts API startup time by ~40% (avoids loading `boto3`, `kubernetes`, etc.).
...
11. **`NodyraItem` typed envelope** — introduce typed item wrapper, update core engine and all built-in nodes.
...
1. **Task Runner process isolation** — the single most important safety feature Nodyra lacks. Code nodes in n8n cannot crash or hang the main process.
2. **DEK-based credential encryption with key rotation** — production-grade secret management.
3. **Scope-based RBAC + Projects** — proper multi-tenancy; Nodyra is single-tenant today.
...
## KEY THINGS NODYRA DOES BETTER (or differently/ahead)
...
8. **`pinned_data`** — frozen node outputs for debugging/testing. n8n has "pin data" too but Nodyra's is persisted server-side.
